...
@section('content')
    <form method="post" action="{{route("orders.display")}}">
        @csrf
        <div class="bg-light p-4 rounded mt-5">
            <div class="container ">
                <div class="row">
                    <div class="col">
                        <img class="product-img" src="{{asset("images/".$data["image_path"])}}
            ...
            <input type="hidden" name="user_id" class="input_user_id"
                   value="{{$data["user_id"]}}"/>
            <button class="btn" type="submit"><strong>Save</strong></button>
        </div>
        <div class="mt-4">
            @role("admin"|"seller")
            <a href="{{ route('products.edit', $data->id) }}" class="btn btn-info">Edit</a>
